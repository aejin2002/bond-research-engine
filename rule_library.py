...
COMMON = {
    ...
    "단, Momentum은 별도 팩터(20% 가중)이면서 동시에 Carry/Value/Yield Curve/Defensive 규칙의 진입조건에도 "
    "공통 필터(Momentum≥0)로 재사용되므로, 8개 팩터가 통계적으로 완전히 독립은 아니다.",
...
RULES = [
    ...
    rule(
        ...
        "또한 이 Rule은 단독 팩터로 그치지 않고 Carry/Value/Yield Curve/Defensive의 +1/-1 판정에도 Momentum≥0 조건으로 "
        "다시 쓰이므로, 강한 모멘텀 하나가 여러 팩터 점수를 동시에 움직여 팩터 가중치(20%)가 시사하는 것보다 총점에 더 크게 기여할 수 있다.",
